...
public class GetSumOfDigitsBetween2Numbers {
    ...
    private static int getRangeDigit(int start, int end) {
    ...
    private static int getDigitSum(int index, int sum, int tight, List<Integer> startDigits) {
        if(index<0) return sum;
        if(dp[index][sum][tight] != -1 && tight !=1) return dp[index][sum][tight];
